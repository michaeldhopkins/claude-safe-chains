...
/// Every sub that localizes when pointed at this machine must do so ONLY for a destination
/// `netloc` positively recognizes. Enumerates the registry, so a service researched
/// later is covered without touching this test.
///
/// This is the one place a value makes a command MORE permissive, so the spoofs matter more here
/// than anywhere else: if `http://localhost@evil.com` cleared the destination facets, a write to an
/// attacker's host would classify as a local edit.
#[test]
fn a_loopback_delta_applies_only_to_a_recognized_local_destination() {
    ...
    for (path, flag) in &subs {
        // With no endpoint flag at all the sub keeps its remote facets, so a write that is
...
///
/// End-to-end the delta is masked: `presents_unlisted_flag` denies a non-loopback endpoint before
/// the facets are ever cleared, so an end-to-end test passes even if the delta itself matched hosts
/// naively. The two layers share `netloc::is_loopback` and must stay INDEPENDENTLY correct — if a
/// future change relaxes admission (a service where a remote endpoint is legitimately allowed),
/// this is what still keeps a remote call from classifying as local.
#[test]
