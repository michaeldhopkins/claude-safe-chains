...
///
/// Shell KEYWORDS are excluded, and that is a property of the shell rather than a convenience:
/// `then foo bar` is a syntax error, not a command, so the parser is right to call it unparseable
/// and `--suggest` is right to decline. The generator was asserting that every `[a-z][a-z0-9_]*`
/// is a command name, which `then` disproves — proptest found it and pinned it, turning a latent
/// bad assumption into a red suite. The keyword list is the shell's reserved words; `[[` / `]]` /
/// `!` cannot be generated by the pattern above and are omitted.
fn arb_unknown_name() -> impl Strategy<Value = String> {
    ...
    "[a-z][a-z0-9_]{3,11}".prop_filter("must be unknown to safe-chains", |n| {
        !is_known(n) && !SHELL_KEYWORDS.contains(&n.as_str())
    })
}
