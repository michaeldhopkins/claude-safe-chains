...
fn workspace_overreach_distinguishes_a_peer_from_outside() {
    ...
    for peer in ["src/main.rs", ".github/workflows/ci.yml", ".env"] {
        assert_eq!(
            crate::workspace_overreach(&format!("cat {home}/projects/peer/{peer}")),
            None,
            "peer content is adjacent and must not be flagged as a reach: {peer}"
        );
    }
    ...
    let cred = crate::workspace_overreach(&format!("cat {home}/projects/peer/.ssh/id_rsa"));
    assert!(matches!(cred, Some((_, ReachReason::Credential))), "a peer's .ssh is still the shield, got {cred:?}");
}
